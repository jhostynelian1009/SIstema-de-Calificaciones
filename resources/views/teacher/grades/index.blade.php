                ...
                @foreach($activities as $act)
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-start py-2 px-3 {{ $isSelected ? 'active bg-primary' : 'bg-white border text-dark' }}"
                           href="{{ route('teacher.assignments.partials.grades.index', [$assignment, $partial, 'activity_id' => $act->id]) }}">
                            <div class="d-flex justify-content-between align-items-center mb-1">
        ...
        <div class="card-header bg-primary text-white py-3 d-flex justify-content-between align-items-center">
            <div>
                <h5 class="card-title mb-0 fw-bold">
                    <i class="bi bi-pencil-square me-2"></i>Matriz de Calificaciones: {{ $selectedActivity->name }} (Ponderación: {{ number_format($selectedActivity->percentage, 2) }}%)
                </h5>
                @if($selectedActivity->description)
                    <small class="text-white-50">{{ $selectedActivity->description }}</small>
                @endif
            </div>
            @if($activityMetrics)
                <span class="badge {{ $activityMetrics['status_badge_class'] }} fs-7">
                    Avance: {{ $activityMetrics['completed_grades'] }} / {{ $activityMetrics['active_students_count'] }} ({{ $activityMetrics['completion_percentage'] }}%)
                </span>
            @endif
        </div>
        ...
        <div class="card-body p-0">
            @if($enrolledStudents->isEmpty())
                <div class="text-center py-5 text-muted">
                    ...
                    No hay estudiantes activos matriculados en este curso y período académico.
                </div>
            @else
                            ...
                            <tbody>
                                @foreach($enrolledStudents as $index => $student)
                                    @php
                                        ...
                                        $scoreValue = old("grades.{$index}.score", $existingGrade ? number_format($existingGrade->score, 2) : '');
                    ...
                    @if(! $isPublished)
                        <div class="card-footer bg-white py-3 text-end border-top">
                            <button type="submit" class="btn btn-primary px-4 fw-bold">
                                <i class="bi bi-save me-1"></i> Guardar Calificaciones de {{ $selectedActivity->name }}
                            </button>
                        </div>
                    @endif
                </form>
